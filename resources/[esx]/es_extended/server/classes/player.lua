function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, status, armour)
	...
	-- Status Giocatore
	function self.createallStatus()
		for k, v in pairs(self.status) do
			self[k] = v
		end
	end

	function self.getStatus(method)
		if method then
			return self.status
		end
		return json.encode(self.status)
	end

	function self.getArmour()
		return self.armour
	end

	function self.setArmour(val)
		if val then
			self.armour = val
		else
			self.armour = 100
	...
	function self.addStatus(index, value)
		if not self.status[index] then
			return false
		end
		if (self.status[index] + value) > 100 then 
			self.setStatus(index, 100)
			return self.status[index] 
		end
		self.status[index] = (self.status[index] + value)
		self.updateStatus()
		return self.status[index]
	end

	function self.removeStatus(index, value)
		if not self.status[index] then	
			return false
		end
		if (self.status[index] - value) <= 0 then
			self.setStatus(index, 0)
			return self.status[index]
		end
		self.status[index] = (self.status[index] - value)
		self.updateStatus()
	...
	function self.setStatus(index, value, setAll)
		if setAll then
			self.status = value
			return true
		end
		if (value > 100) or (value < 0) then return end
		if not self.status[index] then
			return false
		end
	...
	function self.resetStatus(index, setAll)
		if setAll then
			for k, v in ipairs(self.status) do
			...
			if index then
				if self.status[index] then
					self.status[index] = 100
				end
				self.updateStatus()
				return true
			end
	...
	function self.createStatus(index)
		self.status[index] = 100
	end
	
	function self.deleteStatus(index)
		if self.status[index] then
			self.status[index] = nil
		end
	end

	function self.updateStatus()
		self.triggerEvent('dd_hud:updateStatus', self.getStatus(true))
	end
	...
	function self.setJob(job, grade)
		...
		if ESX.DoesJobExist(job, grade) then
			...
			TriggerEvent("dd_extended:updateStateJob", self.source, self.source)
		else
