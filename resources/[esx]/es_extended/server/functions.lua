...
function Core.SavePlayer(xPlayer, cb)
  MySQL.prepare(
    'UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `armour` = ?, `status` = ? WHERE `identifier` = ?',
    {json.encode(xPlayer.getAccounts(true)), xPlayer.job.name, xPlayer.job.grade, xPlayer.group, json.encode(xPlayer.getCoords()),
     json.encode(xPlayer.getInventory(true)), json.encode(xPlayer.getLoadout(true)), GetPedArmour(GetPlayerPed(xPlayer.source)),
     xPlayer.getStatus(), xPlayer.identifier}, function(affectedRows)
...
function ESX.GetInfoPlayer(identifier)
  for k, v in pairs(ESX.Players) do
    if v.identifier == identifier then
      return "online"
    else
      -- Trovare modo per far tornare le informazioni del player offline
    end
  end
end
...
function Core.IsServerOwner(playerId)
...
function ESX.Modder(id, risorsa, msg, code)
  if not code then code = "A-001" end
  print("[^8"..risorsa.."^0] ID:^5"..id.."^0| ^1"..msg.." ^0| Codice: "..code)
end