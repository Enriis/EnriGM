...
function ESX.Dammi(key)
    local valore = ESX.PlayerData[key]
    if valore then
        print("[EXTENDED] Debug | Valore trovato con successo")
        return valore
    else
        print("[EXTENDED] Debug | Valore non trovato")
        return false
    end
end
...
function ESX.ShowNotification(msg, tipo, title, time)
    if not tipo then tipo = "info" end
    exports['dd_notifiche']:showNotify("DD ROLEPLAY", msg, 7000, tipo)  
end
...
AddEventHandler('esx:showNotification', function(msg, type, length)
    exports['dd_notifiche']:showNotify("DD ROLEPLAY", msg, 7000, type)  
end)
