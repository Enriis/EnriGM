...
function OpenPropertyMenu(PropertyId)
  ...
  ESX.OpenContext("right", elements, function(menu, element)
    ...
    if element.value == "property_inventory" then
      ...
      Wait(20)
      ESX.ShowHelpNotification(TranslateCap("storage_pos_textui"))
    ...
    if element.value == "property_wardrobe" then
      ...
      Wait(20)
      ESX.ShowHelpNotification(TranslateCap("wardrobe_pos_textui"))
      while SettingValue ~= "" do
        Wait(1)
        if IsControlJustPressed(0, 47) then
          SettingValue = ""
        ...
        if #(PlayerCoords - vector3(Property.Entrance.x, Property.Entrance.y, 1999)) < 5.0 then
          ...
          if #(PlayerCoords.xy - vector2(Property.Entrance.x, Property.Entrance.y)) <= 2.5 then
            ...
            if not ShowingUIs.Exit then
              ...
              ESX.ShowHelpNotification(TranslateCap("access_textui", Pname))
              ...
              if #(PlayerCoords - Poss) < 5.0 then
                ...
                if #(PlayerCoords - Poss) < 2.0 and SettingValue == "" then
                  ...
                  if not ShowingUIs[k] then
                    ...
                    ESX.ShowHelpNotification(TranslateCap("access_textui", k))
                  end
                  if IsControlJustPressed(0, 38) then
            ...
            if #(Coords - Entrance) < 10.0 then
              ...
              if #(Coords - Entrance) < 1.5 then
                ...
                if not CurrentDrawing.Showing or CurrentDrawing.Name ~= PropertyName then
                  ...
                  ESX.ShowHelpNotification(TranslateCap("access_textui", PropertyName))
                end
                if IsControlJustPressed(0, 38) then
                    OpenPropertyMenu(i)
                end
              ...
              if #(Coords - Garage) < 3.0 then
                ...
                if GetVehiclePedIsUsing(ESX.PlayerData.ped) ~= 0 then
                  ...
                  if not CurrentDrawing.Showing or CurrentDrawing.Name ~= DisplayName then
                    ...
                    CurrentDrawing.Showing = true
                    ESX.ShowHelpNotification(TranslateCap("store_textui", DisplayName))
                  end
                else
                  if not CurrentDrawing.Showing or CurrentDrawing.Name ~= "Garage" then
                    CurrentDrawing.Name = "Garage"
                    CurrentDrawing.Showing = true
                    ESX.ShowHelpNotification(TranslateCap("access_textui", "garage"))
                  end
                end
                if IsControlJustPressed(0, 38) then
                  if GetVehiclePedIsUsing(ESX.PlayerData.ped) ~= 0 then
          ...
          if Dist <= 10.0 then
            ...
            if Dist <= 1.5 then
              if not PMdrawing[k] then
                PMdrawing[k] = true
                ESX.ShowHelpNotification(TranslateCap("realestate_textui", k))
              end
              if IsControlJustPressed(0, 38) then
                OpenPMQuickMenu(k)
              end
        ...
        ESX.OpenContext("right", Property, function(menu, element)
          ...
          if element.index then
            ...
            if element.index == "SetGaragePos" then
              ESX.CloseContext()
              ESX.ShowHelpNotification(TranslateCap("garage_textui"))
              while true do
                Wait(0)
                if IsControlJustPressed(0, 38) then
                  local PlayerPos = GetEntityCoords(ESX.PlayerData.ped)
            ...
            if element.index == "SetCCTVangle" then
              ESX.CloseContext()
              ESX.ShowHelpNotification(TranslateCap("cctv_textui_1"))
              local stage = "angle"
              while true do
                Wait(0)
                if IsControlJustPressed(0, 38) then
                  if stage == "angle" then
                    ...
                    ESX.ShowHelpNotification(TranslateCap("cctv_textui_2"))
                    stage = "maxright"
                  elseif stage == "maxright" then
                    Property[6].value.maxright = GetGameplayCamRot(2).z
                    ESX.ShowHelpNotification(TranslateCap("cctv_textui_3"))
                    stage = "maxleft"
            ...
            ESX.OpenContext("right", GetData(), function(menu, element)
              if element.value then
                ...
                if element.value == "SetGaragePos" then
                  ESX.CloseContext()
                  ESX.ShowHelpNotification("Press ~b~[E]~s~ to Set Position")
                  SettingValue = "Garage"
                  while SettingValue ~= "" do
                    Wait(0)
                    if IsControlJustPressed(0, 38) then
                ...
                if element.value == "SetCCTVangle" then
                  ESX.CloseContext()
                  ESX.ShowHelpNotification("Press ~b~[E]~s~ to Set Angle")
                  local stage = "angle"
                  SettingValue = "cctv"
                  local Angles = {}
                  while stage do
                    ...
                    if IsControlJustPressed(0, 38) then
                      if stage == "angle" then
                        ...
                        ESX.ShowHelpNotification("Press ~b~[E]~s~ to Set Max Right Roation")
                        stage = "maxright"
                      elseif stage == "maxright" then
                        Angles.maxright = GetGameplayCamRot(2).z
                        ESX.ShowHelpNotification("Press ~b~[E]~s~ to Set Max Left Roation")
                        stage = "maxleft"
                      elseif stage == "maxleft" then
