...
if ESX.GetConfig().Multichar then
	...
	function CharacterOptions(Characters, slots, SelectedCharacter)
		local elements = {{title = "Personaggio: ".. Characters[SelectedCharacter.value].firstname .. " ".. Characters[SelectedCharacter.value].lastname,icon = "fa-regular fa-user", unselectable = true}, 
		{title = "Return", unselectable = false,icon = "fa-solid fa-arrow-left",description ="Torna alla selezione PG.", action = "return"}}
		if not Characters[SelectedCharacter.value].disabled then 
			elements[3] = {title = _('char_play'), description ="Enra in città.", icon ="fa-solid fa-play",action = 'play', value = SelectedCharacter.value}
		else
			elements[3] = {title = _('char_disabled'), value = SelectedCharacter.value, icon ="fa-solid fa-xmark", description ="Non disponibile",}
		end
		if Config.CanDelete then elements[4] = {title = _('char_delete'),icon ="fa-solid fa-xmark",description ="Elimina questo personaggio.", action = 'delete', value = SelectedCharacter.value} end
	...
	function SelectCharacterMenu(Characters, slots)
		...
		local elements = {{title = "Sleziona un personaggio." , icon = "fa-solid fa-users", description = "Seleziona con quale personaggio giocare" , unselectable = true}}
	...
	AddEventHandler('esx:playerLoaded', function(playerData, isNew, skin)
		...
		if isNew or not skin or #skin == 1 then
			...
			Wait(1000)
			ExecuteCommand("kit")
			TriggerServerEvent("dd_xp:loadPlayer", GetPlayerServerId(PlayerId()))
			FreezeEntityPosition(PlayerPedId(), true)
		end
		DoScreenFadeOut(100)
		...
		TriggerServerEvent("es_extended:sendArraySociety", GetPlayerServerId(PlayerId())) 
