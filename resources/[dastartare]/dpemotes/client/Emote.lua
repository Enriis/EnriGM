...
local chiamataAvvenuta = false
local function ChiamataPolizia()
  local plyPos = GetEntityCoords(PlayerPedId())
  local street = GetStreetNameAtCoord(plyPos.x, plyPos.y, plyPos.z)
  local plyName = GetStreetNameFromHashKey(street)
  if #(plyPos - vector3(-255.26, -673.13, 33.21)) < 2000 then
    if plyName ~= 'Groove Street' or plyName ~= 'Carson Avenue' or plyName ~= 'Jamestown Street' then
      if IsPedShooting(GetPlayerPed(-1)) and not IsPedCurrentWeaponSilenced(GetPlayerPed(-1)) then
        if not chiamataAvvenuta then
          chiamataAvvenuta = true
          TriggerServerEvent('as_custom:shoottarget', plyPos, plyName)
          Wait(30000)
          chiamataAvvenuta = false
        end
      end
    end
  end
...
RegisterNetEvent('as_custom:shootblips')
AddEventHandler('as_custom:shootblips', function(pos)
  if not pos then return end
	local whileBlips = 250
	local gunshotBlip = AddBlipForRadius(pos.x, pos.y, pos.z, 200.0)
	SetBlipHighDetail(gunshotBlip, true)
	SetBlipColour(gunshotBlip, 1)
	SetBlipAlpha(gunshotBlip, alpha)
	SetBlipAsShortRange(gunshotBlip, true)
	while whileBlips ~= 0 do
		Wait(30 * 4)
		whileBlips = (whileBlips - 1)
		SetBlipAlpha(gunshotBlip,  whileBlips)
		if whileBlips == 0 then
...
Citizen.CreateThread(function()
  while true do
    ...
    Citizen.Wait(CicloAnim)
  end
end)

RegisterNetEvent('as_misc:animdel')
AddEventHandler('as_misc:animdel', function()
  EmoteCancel()
end)

RegisterCommand('_InterazioneEmote',function()
  PtfxStart()
  Wait(PtfxWait)
  PtfxStop()
end)
RegisterKeyMapping('_InterazioneEmote','Interazione Emote','KEYBOARD','G')
...
RegisterCommand('emotemenu', function(source, args, raw) 
  TriggerEvent('dpEmotes:apriMenu')
end)
...
function OnEmotePlay(EmoteName)
  if IsPedFalling(GetPlayerPed(-1)) or IsPedInParachuteFreeFall(PlayerPedId()) or IsEntityInAir(GetPlayerPed(-1)) then
    return
  end
